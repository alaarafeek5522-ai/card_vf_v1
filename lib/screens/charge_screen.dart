...
class _ChargeScreenState extends State<ChargeScreen>
  ...
  Future<void> _confirmAndCharge() async {
    ...
    // تحقق من النجوم
    final stars = await LicenseService.getStars();
    if (stars <= 0) {
      _showNoStarsDialog();
      return;
    }
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                Text('سيتم خصم ⭐ نجمة واحدة',
                  style: GoogleFonts.cairo(color: AppTheme.starColor, fontSize: 13, fontWeight: FontWeight.w600)),
                const SizedBox(height: 16),
                Container(
            ...
            child: Column(
              ...
              children: [
                Container(
                  ...
                  decoration: BoxDecoration(
                    shape: BoxShape.circle,
                    color: AppTheme.starColor.withOpacity(0.1),
                    border: Border.all(color: AppTheme.starColor.withOpacity(0.3), width: 2)),
                  child: const Icon(Icons.star_border_rounded, color: AppTheme.starColor, size: 44)),
                const SizedBox(height: 20),
                Text('لا يوجد نجوم كافية',
                  style: GoogleFonts.cairo(fontSize: 18, fontWeight: FontWeight.w900, color: AppTheme.black)),
                const SizedBox(height: 8),
                Text('رصيدك 0 ⭐\nتحتاج نجمة واحدة لإتمام الشحن',
                  style: GoogleFonts.cairo(color: AppTheme.grey, fontSize: 14), textAlign: TextAlign.center),
                const SizedBox(height: 24),
                SizedBox(width: double.infinity,
                  child: ElevatedButton.icon(
                    style: ElevatedButton.styleFrom(
                      backgroundColor: AppTheme.starColor,
                      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(14)),
                      padding: const EdgeInsets.symmetric(vertical: 14)),
                    icon: const Icon(Icons.shopping_cart_rounded, color: Colors.white),
                    label: Text('شراء نجوم', style: GoogleFonts.cairo(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 15)),
                    onPressed: () { Navigator.pop(context); },
                  )),
                const SizedBox(height: 8),
                TextButton(
                  onPressed: () => Navigator.pop(context),
                  child: Text('إغلاق', style: GoogleFonts.cairo(color: AppTheme.grey))),
              ],
            ),
          ),
        ),
  ...
  Future<void> _charge() async {
    ...
    try {
      ...
      if (ok) {
        // خصم النجمة
        await LicenseService.deductStar();
